...
def process_cmd_files(cmd_files, args, template_path, processed):
    success = 0
    failed = 0
    for cmd_path in cmd_files:
        mtime_ns = cmd_path.stat().st_mtime_ns
        if processed.get(cmd_path) == mtime_ns:
            continue
...
def main():
    ...
    if args.once:
        ...
        success, failed = process_cmd_files(cmd_files, args, template_path, processed)
        log(f"Summary: successful={success} failed={failed}")
        return 0 if failed == 0 else 1

    log("Watching inputs for *_cmd files: " + ", ".join(args.input_paths))
    while True:
        process_cmd_files(find_cmd_files(args.input_paths), args, template_path, processed)
